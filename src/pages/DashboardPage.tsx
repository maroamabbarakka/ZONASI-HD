...
export function DashboardPage() {
  ...
  const canInput = user?.role === 'PK_II' || user?.role === 'PK_III';
  ...
  return <>
    ...
    {openAlerts.length > 0 && <section className="alert-stack"><div className="section-heading"><div><span className="eyebrow"><BellRing /> Early Warning</span><h2>{openAlerts.length} pasien memerlukan perhatian</h2></div></div>{openAlerts.map((alert) => <div className={`alert-item ${alert.type === 'RECENT_RED' ? 'urgent' : ''}`} key={alert.id}><AlertTriangle /><div><strong>{alert.patient_name}</strong><p>{alert.message}</p></div>{['PK_III', 'DOKTER', 'ADMIN'].includes(user!.role) && <button className="button small secondary" onClick={() => acknowledgeAlert(alert.id)}>Tandai ditindaklanjuti</button>}</div>)}</section>}
    ...
    {selected && <Modal title="Input sesi hemodialisis" onClose={() => setSelected(null)} wide><SessionInputForm patient={data.patients.find((p) => p.id === selected.id) ?? selected} role={user!.role} onCancel={() => setSelected(null)} onSave={(form) => { const result = saveSession(selected, form); setSelected(null); setToast(`Sesi ${selected.nama} tersimpan: ${result.idwg_pct.toFixed(1)}% · Zona ${result.zone}`); }} /></Modal>}
