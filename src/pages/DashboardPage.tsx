...
export function DashboardPage() {
  ...
  return <>
    <header className="page-header"><div><span className="eyebrow">Pusat pantau · {now}</span><h1>Situasi pasien terkini</h1><p>Prioritaskan pasien zona Kuning dan Merah, lalu tandai tindak lanjutnya. {dataMode === 'firebase' && dataUpdatedAt ? `Data diperbarui ${new Intl.DateTimeFormat('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(new Date(dataUpdatedAt))}.` : ''}</p></div>{canInput && <button className="button primary" onClick={() => setChoosing(true)}><Plus /> Input sesi baru</button>}</header>
    ...
    {openAlerts.length > 0 && <section className="alert-stack"><div className="section-heading"><div><span className="eyebrow"><BellRing /> Peringatan dini</span><h2>{openAlerts.length} pasien memerlukan perhatian</h2></div></div>{openAlerts.map((alert) => <div className={`alert-item ${alert.type === 'RECENT_RED' ? 'urgent' : ''}`} key={alert.id}><AlertTriangle /><div><strong>{alert.patient_name}</strong><p>{alert.message}</p></div>{canHandleAlerts(user?.role) && <button className="button small secondary" onClick={() => void acknowledgeAlert(alert.id)}>Tandai ditindaklanjuti</button>}</div>)}</section>}
