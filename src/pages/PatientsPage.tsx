...
export function PatientsPage() {
  ...
  return <><header className="page-header"><div><span className="eyebrow">Master dan riwayat · {dataMode === 'firebase' ? 'Firestore real-time' : 'Data demo lokal'}</span><h1>Daftar pasien</h1><p>Nomor RM unik; pasien dengan nama sama tetap dapat dicatat sebagai orang berbeda.</p></div><div className="header-actions">{canImportPatients(user?.role) && <button className="button secondary" onClick={() => setImporting(true)}><FileUp /> Import XLSX</button>}{canManagePatients(user?.role) && <button className="button primary" onClick={() => setEditing('new')}><Plus /> Tambah pasien</button>}</div></header>
    {dataError && <div className="notice danger">Firestore: {dataError}</div>}
    ...
    <div className="table-wrap panel"><table><thead><tr><th>Pasien</th><th>BB Kering</th><th>IDWG terakhir</th><th>Zona</th><th>Streak Kuning</th>{canManagePatients(user?.role) && <th>Aksi</th>}</tr></thead><tbody>{patients.map((patient) => <tr key={patient.id} className="clickable" onClick={() => setSelected(patient)}><td><strong>{patient.nama}</strong><small>{patient.rm}</small></td><td>{patient.bb_kering.toFixed(1)} kg</td><td>{patient.latest_idwg_pct?.toFixed(1) ?? '—'}%</td><td>{patient.latest_zone && <ZoneBadge zone={patient.latest_zone} size="sm" />}</td><td>{patient.yellow_streak}</td>{canManagePatients(user?.role) && <td><button className="icon-button" aria-label={`Edit ${patient.nama}`} onClick={(event) => { event.stopPropagation(); setEditing(patient); }}><Pencil /></button></td>}</tr>)}</tbody></table></div>
    ...
    {importing && <Modal title="Bulk import pasien dari XLSX" onClose={() => setImporting(false)} wide><PatientImport existingRms={data.patients.map((patient) => patient.rm)} onImport={importPatients} onDone={() => setImporting(false)} /></Modal>}
  </>;
}
