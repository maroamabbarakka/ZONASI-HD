...
export function ReportsPage() {
  ...
  return <><header className="page-header"><div><span className="eyebrow">Evaluasi mutu · {dataMode === 'firebase' ? 'Tersambung' : 'Latihan'}</span><h1>Laporan ringkas</h1><p>Periode dan jumlah data ditampilkan agar hasil mudah dibaca.</p></div><div className="header-actions"><label className="field report-period"><span>Periode</span><select value={period} onChange={(event) => setPeriod(event.target.value as Period)}><option value="30">30 hari</option><option value="90">90 hari</option><option value="ALL">Seluruh data</option></select></label>{canExportReports(user?.role) && <button className="button primary" onClick={download}><Download /> Unduh data</button>}</div></header><section className="report-grid"><article><span>Proporsi Sesi Hijau</span><strong>{sessions.length ? Math.round(greenSessions / sessions.length * 100) : 0}%</strong><p>{greenSessions} dari {sessions.length} sesi · {periodLabel}.</p></article><article><span>Rata-rata IDWG</span><strong>{average.toFixed(1)}%</strong><p>Dihitung dari {sessions.length} sesi valid.</p></article><article><span>Kejadian Merah</span><strong>{redSessions}</strong><p>{periodLabel} · dihitung per sesi.</p></article></section><section className="panel"><div className="section-heading"><div><span className="eyebrow">Kondisi terbaru</span><h2>Komposisi zona pasien</h2></div></div><div className="distribution">{(['HIJAU', 'KUNING', 'MERAH'] as const).map((zone) => { const count = active.filter((p) => p.latest_zone === zone).length; const withResult = active.filter((p) => p.latest_zone).length; return <div key={zone}><div><span>{zone}</span><b>{count} dari {withResult} pasien dengan hasil</b></div><div className="bar"><span className={`bar-${zone.toLowerCase()}`} style={{ width: `${withResult ? count / withResult * 100 : 0}%` }} /></div></div>; })}</div></section></>;
}
