...
const protocols: Record<Zone, string[]> = {
  ...
  MERAH: ['Lakukan asesmen kegawatdaruratan segera', 'Aktifkan protokol edema paru unit', 'Kolaborasi dengan PK III dan hubungi DPJP', 'Laksanakan oksigenasi/suction sesuai asesmen, kewenangan, dan SPO RS', 'Pantau tanda vital serta respons pasien', 'Dokumentasikan instruksi medis dan seluruh tindakan'],
};
...
export function ProtocolChecklist({ zone, role, selected, onChange }: { zone: Zone; role: UserRole; selected: string[]; onChange: (items: string[]) => void }) {
  const restricted = zone === 'MERAH' && role === 'PK_II';
  return (
    <div className="protocol-box">
      <div className="section-heading"><h3>Protokol {zone}</h3><span className="eyebrow">Checklist tindakan</span></div>
      {restricted && <div className="notice danger">PK II wajib segera mengeskalasi pasien Zona Merah kepada PK III/DPJP. Checklist tindakan Merah hanya dapat dicatat oleh PK III atau dokter.</div>}
