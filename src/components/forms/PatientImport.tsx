...
export function PatientImport({ existingRms, onImport, onDone }: { existingRms: string[]; onImport: (rows: PatientInput[]) => Promise<{ imported: number; failed: Array<{ rm: string; message: string }> }>; onDone: () => void }) {
  ...
  const parse = async (file: File) => {
    ...
    const parsed = sheet.slice(headerAt + 1).map((row, index): PatientImportRow | null => {
      const values = row.map(text); if (values.every((value) => !value)) return null; const nama = text(row[nameIndex]);
      return { rowNumber: headerAt + index + 2, rm: text(row[rmIndex]), nama, tanggal_lahir: isoDate(row[birthIndex]), jenis_kelamin: genderCode(row[genderIndex]), bb_kering: number(row[dryIndex]), notes: '', errors: [] };
  ...
  const validation = useMemo(() => { const keys = rows.map((row) => rmKey(row.rm)); const databaseKeys = new Set(existingRms.map(rmKey)); return rows.map((row, index) => { const errors: string[] = []; if (!row.rm.trim()) errors.push('RM kosong'); else if (!keys[index]) errors.push('RM tidak valid'); else if (keys.filter((key) => key === keys[index]).length > 1) errors.push('RM duplikat di file'); else if (databaseKeys.has(keys[index])) errors.push('RM sudah ada'); if (row.nama.trim().length < 2 || row.nama.length > 150 || /[\u0000-\u001f\u007f]/.test(row.nama)) errors.push('Nama tidak valid'); if (!isValidDate(row.tanggal_lahir)) errors.push('Tanggal lahir tidak valid'); if (!row.jenis_kelamin) errors.push('Jenis kelamin tidak valid'); if (row.bb_kering < 20 || row.bb_kering > 250) errors.push('BB Kering di luar 20–250 kg'); return errors; }); }, [rows, existingRms]);
  const invalid = validation.filter((errors) => errors.length).length;
  const submit = async () => { setLoading(true); setMessage(''); try { const payload = rows.map(({ rowNumber: _row, errors: _errors, ...row }) => ({ ...row, jenis_kelamin: row.jenis_kelamin as 'L' | 'P' })); const result = await onImport(payload); setMessage(`${result.imported} pasien berhasil diimpor${result.failed.length ? `, ${result.failed.length} gagal.` : '.'}`); if (!result.failed.length) window.setTimeout(onDone, 1200); } catch (error) { setMessage(error instanceof Error ? error.message : 'Import gagal.'); } finally { setLoading(false); } };
  return <div><label className="upload-box"><FileSpreadsheet /><strong>Pilih file Excel</strong><span>File dibaca di perangkat ini dan tidak disimpan sebagai berkas mentah.</span><input type="file" accept=".xlsx" onChange={(event) => { const file = event.target.files?.[0]; if (file) void parse(file).catch((error) => setMessage(error.message)); }} /></label>
