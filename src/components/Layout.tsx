import { Activity, FileChartColumn, Info, LogOut, RotateCcw, ShieldCheck, UserCog, Users } from 'lucide-react';
...
export function Layout() {
  ...
  const links = [
    { to: '/', label: 'Pusat Pantau', mobileLabel: 'Beranda', icon: Activity },
    ...
    { to: '/users', label: user?.role === 'ADMIN' ? 'Pengguna' : 'Akun Saya', mobileLabel: 'Akun', icon: UserCog },
    { to: '/about', label: 'Tentang Aplikasi', mobileLabel: 'Tentang', icon: Info },
  ];
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-footer">
          {dataMode === 'demo' && <button className="button ghost" onClick={resetDemo}><RotateCcw /> Reset data latihan</button>}
          <div className="user-card"><span>{user?.displayName}</span><small>{user ? roleLabels[user.role] : ''} · {user?.uid.startsWith('demo-') ? 'Mode latihan' : 'Akun aktif'}</small></div>
      ...
      <header className="mobile-topbar">
        ...
        <div className="mobile-account">
          <div className="mobile-account-copy"><span>{initials}</span><div><strong>{user?.displayName}</strong><small><i />{dataMode === 'firebase' ? 'Tersambung' : 'Latihan'}</small></div></div>
